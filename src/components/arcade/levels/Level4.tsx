...
// Resistencias con bandas de colores (cada una distinta)
const RESISTORS = [
  {
    id: 'res-330',
  ...
  },
  {
    ...
    value: 10000,
    label: '10kΩ',
    bands: ['marrón', 'negro', 'naranja', 'dorado'], // 10×1000 = 10000
  },
];

// Datos para el modal de ayuda (significado de cada color)
...
export default function Level4() {
  ...
  // Estado para el modal de resistencia
  const [selectedResistor, setSelectedResistor] = useState<typeof RESISTORS[0] | null>(null);

  useEffect(() => {
    setTutorialMsg('🎯 Elige la resistencia correcta (330Ω) entre las opciones. ¡Mira sus bandas de colores!');
    const timer = setTimeout(() => setTutorialMsg(''), 5000);
  ...
  // Manejo de clic en una resistencia (abre modal)
  const handleResistorClick = (item: CircuitComponent | LedComponent) => {
    if (item.type === 'resistor') {
      // Buscar la resistencia en el array RESISTORS para obtener sus bandas
  ...
  const handlePlay = () => {
    ...
    if (boardResistor.value !== 330) {
      setFeedbackMsg(`❌ Resistencia de ${boardResistor.value}Ω no es la correcta. Debe ser 330Ω.`);
      return;
    }
    ...
    setTimeout(() => {
      ...
      setFeedbackMsg('✅ ¡Correcto! La resistencia de 330Ω protege el LED.');
    }, 1500);
  };
  ...
  const handleReset = () => {
    ...
    setSimulationState('idle');
    setFeedbackMsg('');
    setSelectedResistor(null); // cerrar modal si estaba abierto
  };
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-4xl mx-auto relative">
      <h2 className="text-lg font-bold text-white/90 text-center">🎨 Nivel 4 – Código de colores</h2>
      <p className="text-xs text-neutral-500 text-center mb-2">Elige la resistencia de 330Ω mirando sus bandas. Haz clic en una resistencia para ver su código.</p>
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50">
          <div className="relative w-full h-full">
            ...
            {/* Resistencia (usando las bandas del objeto colocado) */}
            <div
                    ...
                    <svg viewBox="0 0 175 100" className="w-full h-auto">
                      ...
                      {boardResistor.bands &&
                        boardResistor.bands.map((color, idx) => {
                          ...
                          const xPos = 65.5 + idx * 10; // ajuste fino
      ...
      {/* Caja de herramientas con soporte para clic en resistencia */}
      <ArcadeToolbox
      ...
      {/* MODAL DE AYUDA PARA RESISTENCIA */}
      <AnimatePresence>
        {selectedResistor && (
          <motion.div
            initial={{ opacity: 0 }}
