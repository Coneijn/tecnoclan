...
const POS = {
  battery: { x: 50, y: 195 },
  resistor: { x: 260, y: 198 }, // Slot central
  led: { x: 452.5, y: 305 },    // Slot derecho
};
...
export default function Level2() {
  ...
  useEffect(() => {
    const showTimer = setTimeout(() => {
      setTutorialMsg(
        '🧩 Arrastra la resistencia y el LED al tablero. ' +
        'La resistencia debe ir ANTES del LED. Si colocas el LED primero, ocupará el lugar de la resistencia y verás el error.'
      );
    }, 300);
  ...
  const handleDragEnd = (e: any, info: any, item: CircuitComponent | LedComponent) => {
    ...
    if (isInsideDropZone) {
      ...
      // --- LED ---
      if (item.type === 'led') {
        // Si la resistencia no está colocada, el LED ocupa el slot de la resistencia.
        if (!boardResistor) {
          setBoardLed({ ...item, x: POS.resistor.x, y: POS.resistor.y } as PlacedLed);
        } else {
          // Si la resistencia ya está, el LED va a su slot normal.
  ...
  // ---------- RESETEAR SIMULACIÓN (al colocar un componente) ----------
  ...
  // ---------- ROTAR LED (haciendo clic sobre él) ----------
  ...
  const handlePlay = () => {
    ...
    // Verificar que la resistencia esté en el slot correcto (antes del LED)
    ...
    // Ahora verificamos polaridad (como en nivel 1)
    ...
    setTimeout(() => {
      ...
      setFeedbackMsg(
        '✅ ¡Perfecto! Colocaste la resistencia antes que el LED, ' +
        'y la polaridad es correcta. El circuito funciona a la perfección.'
      );
    }, 1500);
  };
  ...
  // Componentes disponibles en la caja
    ...
    <div className="flex flex-col h-full w-full max-w-4xl mx-auto relative">
      ...
      <div className="w-full mb-2 mt-1 text-center">
        <h2 className="text-lg md:text-xl font-bold text-white/90 tracking-tight">
          🔢 Nivel 2 – Orden de colocación
        </h2>
        <p className="text-xs text-neutral-500 mt-1">
          Coloca primero la resistencia, luego el LED. Si los intercambias, verás el error.
        </p>
      </div>
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50">
          <div className="relative w-full h-full">
            ...
            {/* Resistencia – puede estar en POS.resistor o POS.led */}
            <div
            ...
            {/* LED – puede estar en POS.led o POS.resistor */}
            <div
