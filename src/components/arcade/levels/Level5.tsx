...
import { Play, RotateCw, AlertCircle, Info, Zap } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { CircuitComponent, LedComponent, SimulationState } from '../types';
import DraggableItem from '../ui/DraggableItem';
import CircuitBoard from '../ui/CircuitBoard';

// Incrementamos el voltaje a 12V
const LEVEL_BATTERY: CircuitComponent = { id: 'bat-12v', type: 'battery', value: 12, label: '12V' };
const LEVEL_LED: LedComponent = { id: 'led-5', type: 'led', polarity: 'correct', color: 'red' }; 

// Resistencias con su código de colores (El objetivo cambia a 470 ohms)
const RES_TARGET: CircuitComponent = { 
  id: 'res-470', type: 'resistor', value: 470, label: '470Ω', 
  bands: ['#eab308', '#9333ea', '#795548', '#ffd700'] // Amarillo, Violeta, Marrón, Oro
};
...
const RES_DISTRACTOR_2: CircuitComponent = { 
  id: 'res-10k', type: 'resistor', value: 10000, label: '10kΩ', 
  bands: ['#795548', '#000000', '#f97316', '#ffd700'] // Marrón, Negro, Naranja, Oro
};
...
export default function Level5() {
  ...
  const handlePlay = () => {
    ...
    setTimeout(() => {
      // Cálculos lógicos simulados
      if (boardResistor.value === 220) {
        setSimulationState('exploded');
        setFeedbackMsg('¡BOOM! Usaste 220Ω, pero con 12V necesitas una resistencia mayor. La corriente destruyó el LED.');
      } else if (boardResistor.value === 10000) {
        setSimulationState('off');
        setFeedbackMsg('¡10kΩ es muchísima resistencia! Los electrones no tienen la fuerza para pasar y el LED no enciende.');
      } else {
        setSimulationState('perfect');
        setFeedbackMsg('¡Magnífico! Con 12V, la resistencia de 470Ω (Amarillo, Violeta, Marrón) es la ideal para proteger el LED.');
      }
    }, 1500); 
  };
    ...
    <div className="p-4 md:p-8 flex flex-col items-center">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full max-w-6xl">
        ...
        <div className={`col-span-1 bg-neutral-900 border-2 rounded-xl p-6 shadow-xl transition-all duration-500 ${isSimulating ? 'opacity-50 pointer-events-none border-neutral-800' : 'border-neutral-700'}`}>
          ...
          <div className="bg-orange-950/50 border border-orange-900 p-4 rounded-lg mb-6 flex gap-3">
            <Zap className="text-orange-400 shrink-0" />
            <p className="text-sm text-orange-200">
              ¡Cuidado! Subimos el voltaje a <strong>12V</strong>. <br/><br/>
              Una resistencia de 330Ω ya no es suficiente. Busca la resistencia de <strong>470Ω</strong> para proteger el LED.<br/><br/>
              Colores: <span className="text-yellow-500 font-bold">Amarillo (4)</span>, <span className="text-purple-400 font-bold">Violeta (7)</span>, <span className="text-amber-600 font-bold">Marrón (x10)</span>.
            </p>
          </div>
        ...
        <div className="col-span-1 lg:col-span-2 flex flex-col">
          <CircuitBoard dropZoneRef={dummyBoardRef} isSimulating={isSimulating} simulationState={simulationState} ledPolarity={boardLed?.polarity}>
            ...
            {/* ZONA BATERÍA */}
            <div ref={batZoneRef} className={`absolute left-[2%] top-1/2 -translate-y-1/2 w-24 h-32 z-10 flex items-center justify-center rounded-xl transition-colors ${!boardBattery ? 'border-2 border-dashed border-orange-600/50 bg-orange-900/10' : ''}`}>
              <AnimatePresence>
                {boardBattery && (
