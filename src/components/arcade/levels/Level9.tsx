  ...
  const handlePlay = () => {
    ...
    setTimeout(() => {
      if (boardLed.polarity === 'reversed') {
        ...
        setFeedbackMsg('Incluso en papel, ¡la polaridad importa! El cátodo (-) está colocado al revés.');
      } else {
        setSimulationState('perfect');
        setFeedbackMsg('¡Reto Completado! Ahora eres capaz de diseñar y leer diagramas esquemáticos profesionales.');
      }
    }, 1500); 
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full max-w-6xl">
        ...
        <div className={`col-span-1 bg-neutral-900 border-2 rounded-xl p-6 shadow-xl transition-all duration-500 ${isSimulating ? 'opacity-50 pointer-events-none border-neutral-800' : 'border-neutral-700'}`}>
          <div className="flex items-center gap-2 mb-2 border-b border-neutral-700 pb-2">
            <h2 className="text-xl font-semibold">100% Esquemático</h2>
          </div>
          
          <div className="bg-rose-950/50 border border-rose-900 p-4 rounded-lg mb-6 flex flex-col gap-3">
            <div className="flex items-center gap-2 text-rose-300 font-bold border-b border-rose-800/50 pb-2">
              <BookOpen size={18} /> Diagrama Profesional
            </div>
            <p className="text-sm text-rose-200">
              Ya no hay componentes físicos de ayuda. El menú de insumos y el circuito se componen por símbolos esquemáticos puros. 
            </p>
          </div>

          <div className="space-y-6">
            <div className="flex gap-4 items-start">
              {/* Le pasamos mode="schematic" a los arrastrables */}
              {!boardBattery && <DraggableItem item={LEVEL_BATTERY} onDragEnd={handleDragEnd} mode="schematic" />}
              {!boardResistor && <DraggableItem item={LEVEL_RESISTOR} onDragEnd={handleDragEnd} mode="schematic" />}
            </div>
            
            {!boardLed && (
              <div className="pt-4 border-t border-neutral-800">
                <h3 className="text-xs font-bold text-neutral-500 mb-4 uppercase tracking-wider">LEDs (Esquemáticos)</h3>
                <div className="flex flex-wrap gap-4">
                  <DraggableItem item={LED_CORRECT} onDragEnd={handleDragEnd} mode="schematic" />
                  <DraggableItem item={LED_REVERSED} onDragEnd={handleDragEnd} mode="schematic" />
                </div>
              </div>
            )}
          </div>
