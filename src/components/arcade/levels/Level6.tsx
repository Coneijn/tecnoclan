...
const LEVEL_LED: LedComponent = { id: 'led-6', type: 'led', polarity: 'correct', color: 'red' }; 

// Resistencias con valor (para que elijan la correcta por cálculo)
const RES_TARGET: CircuitComponent = { 
  id: 'res-330', type: 'resistor', value: 330, label: '330Ω', 
  bands: ['#f97316', '#f97316', '#795548', '#ffd700'] 
};
const RES_DISTRACTOR_1: CircuitComponent = { 
  id: 'res-100', type: 'resistor', value: 100, label: '100Ω', 
  bands: ['#795548', '#000000', '#795548', '#ffd700'] 
};
const RES_DISTRACTOR_2: CircuitComponent = { 
  id: 'res-1k', type: 'resistor', value: 1000, label: '1kΩ', 
  bands: ['#795548', '#000000', '#ef4444', '#ffd700'] 
};
...
export default function Level6() {
  ...
  // Estado para el modal de la calculadora matemática
  ...
  const handlePlay = () => {
    ...
    setTimeout(() => {
      ...
      if (boardResistor.value === 100) {
        ...
        setFeedbackMsg('¡BOOM! 100Ω permite demasiada corriente (70mA). Superaste el límite seguro del LED (20mA).');
      } else if (boardResistor.value === 1000) {
        setSimulationState('off');
        setFeedbackMsg('1kΩ limita la corriente a solo 7mA. Es seguro, pero el LED casi no ilumina. ¡Busca el valor óptimo!');
      } else {
        setSimulationState('perfect');
        setFeedbackMsg('¡Felicidades! 330Ω es el valor comercial más cercano a los 350Ω calculados. ¡Corriente perfecta (21mA)!');
      }
    }, 1500); 
  };
  ...
  return (
    <div className="p-4 md:p-8 flex flex-col items-center relative">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full max-w-6xl">
        ...
        <div className={`col-span-1 bg-neutral-900 border-2 rounded-xl p-6 shadow-xl transition-all duration-500 ${isSimulating ? 'opacity-50 pointer-events-none border-neutral-800' : 'border-neutral-700'}`}>
          <div className="flex items-center gap-2 mb-2 border-b border-neutral-700 pb-2">
            <h2 className="text-xl font-semibold">Cálculo: Ley de Ohm</h2>
          </div>
          
          <div className="bg-indigo-950/50 border border-indigo-900 p-4 rounded-lg mb-6 flex flex-col gap-4">
            
            {/* Fórmula estilo Fracción */}
            <div className="flex items-center gap-3 text-indigo-300 font-bold border-b border-indigo-800/50 pb-3">
              <Calculator size={20} /> 
              <div className="flex items-center gap-2 text-lg">
                <span>R =</span>
                <div className="flex flex-col items-center leading-none">
                  <span className="border-b-2 border-indigo-400 px-2 pb-1">V - V_led</span>
            ...
            <p className="text-sm text-indigo-200">
              Usa la fórmula para encontrar la resistencia (<strong>R</strong>) óptima. <br/><br/>
              • Fuente (<strong>V</strong>) = 9 Voltios<br/>
              • Consumo LED (<strong>V_led</strong>) = 2 Voltios<br/>
              • Corriente ideal (<strong>I</strong>) = 0.02 Amperios (20mA)<br/><br/>
              <em className="text-indigo-400">Nota: Elige la resistencia que más se acerque al resultado.</em>
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            {!boardResistor && (
              <div className="pt-4 border-t border-neutral-800">
                ...
                {/* Título de Caja de Resistencias + Triangulito */}
                <div className="flex items-center gap-3 mb-4">
                  <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-wider mb-0">Caja de Resistencias</h3>
                  
                  <button 
                    onClick={() => setIsCalcOpen(true)}
                    className="group relative flex items-center justify-center text-neutral-500 hover:text-indigo-400 transition-colors"
                  >
      ...
      {/* Modal de la Calculadora de Apoyo */}
      <WidgetModalWrapper isOpen={isCalcOpen} onClose={() => setIsCalcOpen(false)} isModal={true}>
        {/* Cabecera inyectada en el modal para ayudar con el cálculo específico */}
        <div className="mb-4 p-5 bg-indigo-50 border border-indigo-100 rounded-xl text-center shadow-sm">
          <p className="text-xs font-bold text-indigo-500 uppercase tracking-wider mb-2">Ecuación a resolver</p>
          <div className="flex items-center justify-center gap-3 font-mono text-xl text-indigo-900 font-bold">
            <span>R =</span>
            <div className="flex flex-col items-center leading-none">
              <span className="border-b-2 border-indigo-900 px-3 pb-1">9 - 2</span>
